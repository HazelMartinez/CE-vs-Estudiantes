...
Lista<Enemigo *> Evolucionador::CreaNuevaGeneracion(Lista<Enemigo *> actual)
{
    Lista<Enemigo *> nuevaGen;
    Node<Enemigo*> *temp = actual.head;
    Node<Enemigo*> *mayor = actual.head;
    for(int i = 0; i<actual.len/2;i++){
        ...
        mayor->Data->fitness = 0;
        nuevaGen.add_end(mayor->Data);
        Node<Enemigo*> * temp = nuevaGen.head;
        while (temp->next!= nullptr) {
            nuevaGen.add_end(this->Cruce(temp->Data,temp->next->Data));
            temp = temp->next->next;


        }
        for (int i = 0; i<2; i++){
            nuevaGen.add_end(this->Mutacion(nuevaGen.get(rand() % (nuevaGen.len))));
        }
        for (int i = 0; i<1; i++){
             nuevaGen.add_end(this->Inversion(nuevaGen.get(rand() % (nuevaGen.len))));
